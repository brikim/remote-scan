...
services:
  remotescan:
    container_name: remote-scan
    image: brikim/remote-scan:latest
    security_opt:
...
| Volume | Function |
| :------- | :------------------------ |
| /config  | Path to a folder containing config.yml used to setup Remote-Scan |
| /logs    | Path to a folder to store Remote-Scan log files |
| /media   | Path to your media files. Used to scan directories for changes |

### Configuration File
A configuration file is required to use Remote-Scan. Create a config.conf file in the volume mapped to /config
...
| key              | Key to be used to send notifications |
